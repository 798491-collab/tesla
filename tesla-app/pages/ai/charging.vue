  ...
  <view class="page" :class="themeClass">
    ...
    <scroll-view scroll-y class="main-scroll" @scrolltolower="loadMore">
      <view class="filter-bar">
        ...
        <view class="filter-item" :class="{ active: filter === 'charging' }" @click="filter = 'charging'; resetList()">单次充电</view>
        <view class="filter-item" :class="{ active: filter === 'charging_monthly' }" @click="filter = 'charging_monthly'; resetList()">月度汇总</view>
      </view>
      ...
      <view class="report-list" v-if="list.length > 0">
        <view class="report-card" v-for="item in list" :key="item.id" @click="toggleExpand(item.id)">
          <view class="report-header">
            <view class="report-type-badge" :class="item.ref_id.startsWith('charging_monthly:') ? 'monthly' : 'single'">
              <Icon :name="item.ref_id.startsWith('charging_monthly:') ? 'Calendar' : 'Flash'" :size="12" color="#fff" />
              <text class="badge-text">{{ item.ref_id.startsWith('charging_monthly:') ? '月度' : '单次' }}</text>
            </view>
      ...
      <view class="empty-state" v-if="!loading && list.length === 0">
        ...
        <text class="empty-text">暂无充电AI分析记录</text>
        <text class="empty-hint">完成充电后将自动生成分析报告</text>
      </view>
...
const loadList = async () => {
  ...
  try {
    ...
    const res = await getAnalysisList(vin.value, 'charging', params.page, params.page_size)
...
const goToRecord = (item) => {
  ...
  if (refId.startsWith('charging:')) {
    const date = new Date(item.created_at)
    const month = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`
    uni.navigateTo({ url: `/pages/charging/month?vin=${vin.value}&month=${month}` })
  } else if (refId.startsWith('charging_monthly:')) {
    const month = refId.replace('charging_monthly:', '')
    uni.navigateTo({ url: `/pages/charging/month?vin=${vin.value}&month=${month}` })
  }
}

const formatRefId = (refId) => {
  if (refId.startsWith('charging_monthly:')) return refId.replace('charging_monthly:', '')
  if (refId.startsWith('charging:')) return '充电 #' + refId.replace('charging:', '')
  return refId
}
